...
void GRANDEventVoltage::TraceADC2Voltage(int du_num, GRANDEventADC *adc)
{
	...
	x[du_num].resize(adc->trace_0[du_num].size());
	transform(adc->trace_0[du_num].begin(), adc->trace_0[du_num].end(), x[du_num].begin(), [adc2voltageconst](short &c){ return c*adc2voltageconst; });
	y[du_num].resize(adc->trace_1[du_num].size());
	transform(adc->trace_1[du_num].begin(), adc->trace_1[du_num].end(), y[du_num].begin(), [adc2voltageconst](short &c){ return c*adc2voltageconst; });
	z[du_num].resize(adc->trace_2[du_num].size());
	transform(adc->trace_2[du_num].begin(), adc->trace_2[du_num].end(), z[du_num].begin(), [adc2voltageconst](short &c){ return c*adc2voltageconst; });
}
